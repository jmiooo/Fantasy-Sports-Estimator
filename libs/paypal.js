...
function createPayment(user, cardType, cardNumber, expireMonth, expireYear, cvv2, firstName, lastName, line1, city, state, postalCode, countryCode, amount, callback) {
  makePaymentCreditCard(cardType, cardNumber, expireMonth, expireYear, cvv2, firstName, lastName, line1, city, state, postalCode, countryCode, amount, function(err, result) {
    if (err) {
      callback(err)
    }
    else {
      var paymentJson = result;
      paypal_api.payment.create(paymentJson, config_opts, function(err, res) {
        if (err) {
          console.log(err);
        }
        else {
          User.updateMoney([amount], user, function(err) {
            if (err) {
              callback(err);
            }
          })
